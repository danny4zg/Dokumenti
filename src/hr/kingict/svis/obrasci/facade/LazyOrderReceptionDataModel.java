...
public class LazyOrderReceptionDataModel extends LazyDataModel<Assign2Bean>{
	...
	public LazyOrderReceptionDataModel(){
	...
	public LazyOrderReceptionDataModel(Map<String, Object> searchParam){	
		FacesContext context = FacesContext.getCurrentInstance();
		ub = (UserBean) context.getExternalContext().getSessionMap().get("user");
	...
	private List<Assign2Bean> getPageData(Integer min, Integer max){
    	...
    	try {
        	...
        	if (search == false){
            	...
            	trx = ctxA.getDatabaseManager().createTransaction("sys");
            	rs =  trx.executePreparedQueryById("test", "SearchDeliveryOrder", parm);
			...
			while (rs.next()){
				...
				doc.setPrice(rs.getBigDecimal(14));
				ab.setDoc(doc);
	...
	private int rowCount(){
    	...
    	try {
			...
			if (search == true) {
				...
				rs =  trx.executePreparedQueryById("test", "CountSearchDeliveryOrder", parm);
			}
			else {
				parm.add(ub.getIdOrganizacije());
