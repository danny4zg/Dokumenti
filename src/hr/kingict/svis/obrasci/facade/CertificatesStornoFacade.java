...
@ManagedBean(name="certStorno")
@ViewScoped
public class CertificatesStornoFacade implements Serializable {
	...
	public CertificatesStornoFacade(){
		...
		try {
			...
			if (rola != 1200 && rola != 1227 && !isAdmin && rola != 2 && rola != 12 && rola != Const.ROLA_DVI.intValue() && rola != Const.ROLA_INSPEKTOR.intValue()){
	    		...
	    		lazyModel = new LazyCertPrintedDataModel(true);
	...
	public void generateExcel(){
		String template = "report_stornirani";
		String output_name = "stornirani_certifikati";

		try {
			Map<Integer, Object> map = new HashMap<Integer, Object>();
			if (rola != 1200 || rola != 1227) map.put(1, ub.getIdOrganizacije());
			else map.put(1, Const.SEARCH_MORE);
			fileDownload = ReportUtil.generateExcelReportAdvanced("ReportStorniraniCertifikati", template, output_name, map);
	...
	public void search(){
		...
		try {
			...
			lazyModel = new LazyCertPrintedDataModel(map, selectedMjera, selectedJezici, selectedDrzava, true);
		}
		catch (Exception e){
			e.printStackTrace();
		}
