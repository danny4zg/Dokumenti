...
public class LazyContingentDataModel extends LazyDataModel<Contingent>{
	
	private static final long serialVersionUID = 1L;
	protected IApplicationContext ctxA = null;
	
	public LazyContingentDataModel(){
	...
	@Override  
    public List<Contingent> load(int first, int pageSize, String sortField, SortOrder sortOrder, Map<String,String> filters) {  
	...
	private List<Contingent> getPageData(Integer min, Integer max){
    	List parm = new ArrayList();
    	List<Contingent> lobj = new ArrayList<Contingent>();
    	...
    	try {
			...
			rs =  trx.executePreparedQueryById("test", "SelectBarcodeContingent", parm);
			...
			while (rs.next()){
				...
				o = new Contingent();
				o.setRownum(rs.getInt(1));
				o.setIdContingent(rs.getInt(2));
				o.setBarcodeTypeCode(rs.getInt(3));
				o.setBarcodeType(rs.getString(4));
				o.setCreateDate(rs.getDate(5));
				o.setAmount(rs.getInt(6));
				o.setBarcodeFrom(rs.getInt(7));
				o.setBarcodeTo(rs.getInt(8));
				o.setAvailable(rs.getInt(9));
	...
	private int rowCount(){
    	...
    	try {
        	...
        	trx = ctxA.getDatabaseManager().createTransaction("sys");
			rs =  trx.executePreparedQueryById("test", "CountBarcodeContingent", parm);
