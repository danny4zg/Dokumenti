...
@ManagedBean(name="orderDeliv")
@ViewScoped
public class OrderDeliveryFacade implements Serializable {
	...
	public OrderDeliveryFacade(){
		...
		try {
			...
			if (rola != 3){
				...
				lazyModel = new LazyOrderDeliveryDataModel();
			}	
	...
	public void search(){
		try {
			...
			lazyModel = new LazyOrderDeliveryDataModel(map);
		...
		try {
			...
			for (Assign2Bean o : (List<Assign2Bean>) lazyModel.getWrappedData()){
				if (o.getIdNarudzbe().intValue() == this.selected.intValue()){
					...
					context.getExternalContext().getSessionMap().put("deliord", o);
					context.getExternalContext().getSessionMap().put("iddelord", this.selected);
					break;
				}
			}
			
	        FacesContext.getCurrentInstance().getApplication().getNavigationHandler().handleNavigation(FacesContext.getCurrentInstance(), null, "edit_order_delivery?faces-redirect=true");
	 	}
	 	catch (Exception ex){
