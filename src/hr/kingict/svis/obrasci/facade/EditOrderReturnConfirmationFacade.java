...
@ManagedBean(name="editOrdReturnConf")
@ViewScoped
public class EditOrderReturnConfirmationFacade implements Serializable {
	private static final long serialVersionUID = 1L;
	protected IApplicationContext ctxA = null;
	private ArrayList<SelectItem> pdocList = new ArrayList<SelectItem>();
	private ArrayList<SelectItem> naruciteljList = new ArrayList<SelectItem>();
	private Integer selected;
	private Integer selectedNarucitelj;
	...
	public EditOrderReturnConfirmationFacade(){
		...
		try {
			...
			if (rola != 3){
				...
				parm.add(ub.getIdOrganizacije());
				rs =  trx.executePreparedQueryById("test", "OrderReturnConfirm", parm);
				
				boolean insert;
				boolean isFirst = true;
				Integer tmpNarucitelj = null;
				boolean empty = true;
				...
				while (rs.next()){
					...
					insert = true;
					for (SelectItem x: naruciteljList){
						if (((Integer) x.getValue()).intValue() == rs.getInt(11)) {
							insert = false;
							break;
						}
					}
					...
					if (insert) {
						naruciteljList.add(new SelectItem(rs.getInt(11), rs.getString(14)));
						if (isFirst){
							tmpNarucitelj = rs.getInt(11);
							isFirst = false;
						}
					}
					
					if (tmpNarucitelj == rs.getInt(11)){
				...
				if (empty){
					FacesMessage message = new FacesMessage(FacesMessage.SEVERITY_INFO, "Nema vraćenih dokumenata",  "");    
					...
					if (pdocList.size() == 0) {
						...
						for (Assign2Bean doc : currList){
							if (tmpNarucitelj.intValue() == doc.getIdNarucitelj().intValue()){
	...
	public void naruciteljValueChange(){
		pdocList.clear();
		boolean first = true;
		for (Assign2Bean as : currList){
			if (selectedNarucitelj.intValue() == as.getIdNarucitelj().intValue()){
				pdocList.add(new SelectItem(as.getIdStavkaNarudzbe(), as.getDoc().getCode()));
				if (first){
					first = false;
					this.selected = as.getIdStavkaNarudzbe();
					this.selDoc = as;
				}
			}
		}
	...
	public void add(ActionEvent event){
		...
		try {
			...
			a1.setSerijaDo(a1.getStavka().getSerijskiBrojOd());
			a1.setDatum(datum);
			assignList.add(a1);
	...
	public void confirm(){
		...
		try {
    		...
    		for (Assign4Bean a : assignList){
    			...
    			parm.clear();
    			parm.add(Const.PRIMITAK_VRACENO);
    			parm.add(a.getStavka().getIdStavkaNarudzbe());
	...
	public ArrayList<SelectItem> getNaruciteljList() {
		return naruciteljList;
	}

	public void setNaruciteljList(ArrayList<SelectItem> naruciteljList) {
		this.naruciteljList = naruciteljList;
	}

	public Integer getSelectedNarucitelj() {
		return selectedNarucitelj;
	}

	public void setSelectedNarucitelj(Integer selectedNarucitelj) {
		this.selectedNarucitelj = selectedNarucitelj;
	}
