...
@ViewScoped
public class EstablishmentFacade implements Serializable {
	private static final long serialVersionUID = 1L;
	protected IApplicationContext ctxA = null;
	private LazyDataModel<ObjektExt> lazyModel;
	...
	public EstablishmentFacade(){
		...
		if (rola != 1226 && rola != 1012){
			...
			lazyModel = new LazyEstablishmentDataModel();
		}
	}
	...
	public void search(ActionEvent event){
		try {
			...
			lazyModel = new LazyEstablishmentDataModel(map);
		}
		catch (Exception e){
			e.printStackTrace();
		}
	...
	public String getDetail(){  
		...
		try {
			...
			for (ObjektExt o : (List<ObjektExt>) lazyModel.getWrappedData()){
				if (o.getIdObjekt().intValue() == this.selected.intValue()){

					context.getExternalContext().getSessionMap().put("est", o);
					break;
				}
			}
	...
	public LazyDataModel<ObjektExt> getLazyModel() {
		return lazyModel;
	}

	public void setLazyModel(LazyDataModel<ObjektExt> lazyModel) {
