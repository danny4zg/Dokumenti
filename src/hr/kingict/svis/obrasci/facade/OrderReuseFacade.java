...
@ManagedBean(name="orderReuse")
@ViewScoped
public class OrderReuseFacade implements Serializable {
	...
	public OrderReuseFacade(){
		...
		try {
			...
			if (rola != 3){
				...
				lazyModel = new LazyOrderReuseDataModel();
			}	
	...
	public void search(){
		...
		try {
			...
			lazyModel = new LazyOrderReuseDataModel(map);
	...
	public String getDetail(){   	
		try {
			...
			for (Assign2Bean o : (List<Assign2Bean>) lazyModel.getWrappedData()){
				if (o.getIdStavkaNarudzbe().intValue() == this.selected.intValue()){

					context.getExternalContext().getSessionMap().put("reuseord", o);
					context.getExternalContext().getSessionMap().put("residord", this.selected);
					break;
				}
			}
			
	        FacesContext.getCurrentInstance().getApplication().getNavigationHandler().handleNavigation(FacesContext.getCurrentInstance(), null, "edit_order_reuse?faces-redirect=true");
	 	}
	 	catch (Exception ex){
