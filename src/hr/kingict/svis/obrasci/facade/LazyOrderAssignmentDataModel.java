...
public class LazyOrderAssignmentDataModel extends LazyDataModel<Assign2Bean>{
	...
	public LazyOrderAssignmentDataModel(){
	...
	public LazyOrderAssignmentDataModel(Map<String, Object> searchParam){	
		FacesContext context = FacesContext.getCurrentInstance();
		ub = (UserBean) context.getExternalContext().getSessionMap().get("user");
	...
	private List<Assign2Bean> getPageData(Integer min, Integer max){
    	...
    	try {
        	...
        	if (search == false){
	        	...
	        	rs =  trx.executePreparedQueryById("test", "SelectAssignOrder", parm);
        	}
        	else {
            	...
            	if (this.activeTab == Const.CODE_DEFAULT.intValue()){
            		rs =  trx.executePreparedQueryById("test", "SearchAssignOrderAll", parm);
        		}
        		else if (this.activeTab == Const.CODE_ORG.intValue()){
        			rs =  trx.executePreparedQueryById("test", "SearchAssignOrderType1", parm);
        		}
        		else if (this.activeTab == Const.CODE_OBJEKT.intValue()){
        			rs =  trx.executePreparedQueryById("test", "SearchAssignOrderType2", parm);
        		}
	...
	private int rowCount(){
    	...
    	try {
			...
			if (search == true) {
        		...
        		if (this.activeTab == Const.CODE_DEFAULT.intValue()){
            		rs =  trx.executePreparedQueryById("test", "CountSearchAssignOrderAll", parm);
        		}
        		else if (this.activeTab == Const.CODE_ORG.intValue()){
        			rs =  trx.executePreparedQueryById("test", "CountSearchAssignOrderType1", parm);
        		}
        		else if (this.activeTab == Const.CODE_OBJEKT.intValue()){
        			rs =  trx.executePreparedQueryById("test", "CountSearchAssignOrderType2", parm);
        		}
			}
			else {
				parm.add(ub.getIdOrganizacije());
				rs =  trx.executePreparedQueryById("test", "CountAssignOrder", parm);
			}
