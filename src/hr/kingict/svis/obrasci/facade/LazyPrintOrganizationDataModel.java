...
public class LazyPrintOrganizationDataModel extends LazyDataModel<OrganizationBean>{
	...
	public LazyPrintOrganizationDataModel(){
		search = false;
	}
	
	public LazyPrintOrganizationDataModel(Map<String, Object> searchParam){
	...
	private List<OrganizationBean> getPageData(Integer min, Integer max){
    	...
    	try {
        	...
        	if (search == false){
            	...
            	trx = ctxA.getDatabaseManager().createTransaction("sys");
    			rs =  trx.executePreparedQueryById("test", "SearchTiskare", parm);
	...
	private int rowCount(){
    	...
    	try {
			...
			if (search == true) {
				...
				rs =  trx.executePreparedQueryById("test", "CountSearchTiskare", parm);
			}
			else {
