...
@ManagedBean(name="editOrdRec")
@ViewScoped
public class EditOrderReceptionFacade implements Serializable {
	...
	public EditOrderReceptionFacade(){
		...
		try {
			...
			if (rola != 5 && rola != 2 && rola != 12 && rola != 2 && rola != 1226 && rola != 1228){
				...
				if (rola == Const.ROLA_OBJEKT.intValue()){
	    			rs =  trx.executePreparedQueryById("test", "OrderReceptionObjekt", parm);
		    	}
		    	else { 
	    			rs =  trx.executePreparedQueryById("test", "OrderReceptionOrg", parm);
		    	}
	...
	public void confirm(){
		...
		try {
    		...
    		for (Assign4Bean a : assignList){	
    			...
    			parm.clear();
    			rs =  trx.executePreparedQueryById("test", "GetSequenceReceivedItem", parm);
    			...
    			trx.executePreparedUpdateById("test", "InsertReceivedItem", parm);
    			trx.commit();
    		
    			parm.clear();
    			parm.add(Const.PRIMLJENO);
    			parm.add(a.getStavka().getIdStavkaNarudzbe());
    			...
    			if (a.getStavka().getDoc().getCode().equals(Const.BARKOD)){
    				idNarudzba = a.getStavka().getIdStavkaNarudzbe();
	    			parm.clear();
					parm.add(idNarudzba);
					parm.add(ub.getIdOrganizacije());
					trx.executePreparedUpdateById("test", "UpdateTubeStatus", parm);
					trx.commit();
    			}
    		}
	        step = 2;
	...
	public void checkConfirmStatus(ActionEvent actionEvent){  
		...
		try {
			...
			FacesMessage message = new FacesMessage(FacesMessage.SEVERITY_INFO, "Potvrdili ste prijem dokumenata",  "");    
			FacesContext.getCurrentInstance().addMessage(null, message);  
		}
		catch (Exception e){
			e.printStackTrace();
